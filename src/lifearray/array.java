...
public class array {
	...
	/**
	 * 世代交替。 生命游戏的核心是在当前world数据基础上计算出下一代的world数据，以便打印新一代的二维世界。
	 * 为此，需扫描当代world的每一个格子，计算其邻居的数量，以条件语句表达生命演化规则，设定新一代world的数据。
	 * 因为在所有的格子计算完之前，world的数据不得变化， 因此使用了应该临时的temp，暂存二维世界的数据。 ★calculate()
	 */
	public void nextWorld() {
		...
		/*
		 * for (int height = 0; height < HEIGHT; height++) { for (int width = 0;
		 * width < WIDTH; width++) { int neighbor = getNeighbors(height, width);
		 * if (neighbor==3) { temp[height][width] = Y; }else if (neighbor == 2
		 * && world[height][width] == Y){ temp[height][width] = Y; }else{
